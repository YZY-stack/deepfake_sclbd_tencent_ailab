...
from albumentationsimport torch
import os
import cv2
import albumentations as A
        ...
        for i, d in enumerate(dataset_img.datasets):
            ...
            for img, label in dataloader:
                ...
                if not TTA:
                    ...
                    pred = [np.mean(x) for x in chunked(tmp_pred, len(transforms))]
                    true = [np.mean(x) for x in chunked(tmp_true, len(transforms))]
                    if video_auc:
                        # video
...
def evaluate(model, data_path, mode='val'):
    ...
    # dataset_img = torch.utils.data.ConcatDataset([dataset_real, dataset_fake])
    # *** celeb *** #
    dataset_real = CelebDataset(dataset_root=real_root, size=256, frame_num=50, augment=True)
    dataset_fake = CelebDataset(dataset_root=fake_root, size=256, frame_num=50, augment=True)
    dataset_img = torch.utils.data.ConcatDataset([dataset_real, dataset_fake])
